<script>
import { inject } from 'vue';

export default {
  name: 'AdminTagList',
  components: {},
  setup() {
    const tags = inject('tags');
    console.log(tags);
    return { tags };
  },
};
</script>

<template>
  <section class="mt-6">
    <h2 class="text-xl font-semibold text-gray-700">Tag List</h2>
    <div class="mt-4 grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      <div
        v-if="tags.length"
        v-for="tag in tags"
        :key="tag.id"
        class="p-4 bg-white shadow rounded-md"
      >
        <!-- Flex Container für Tag und Button -->
        <div class="flex items-center justify-between">
          <!-- Tag Name (mit Doppelklick zum Bearbeiten) -->
          <div
            class="bg-blue-100 text-blue-600 shadow rounded-md py-1 px-2 cursor-pointer"
            v-if="!tag.isEditing"
            @dblclick="tag.isEditing = true"
          >
            {{ tag.data.name }}
          </div>
          <input
            v-else
            v-model="tag.data.name"
            @blur="tag.isEditing = false"
            class="border p-1 rounded-md"
          />
          <div class="ml-4 text-sm text-gray-500">
            {{ tag.data.projectCount }} {{ tag.data.projectCount > 1 || tag.data.projectCount == 0 ? 'Projects' : 'Project' }}
          </div>

          <!-- Delete Button -->
          <button class="ml-4 text-red-600 hover:text-red-800">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
              stroke="currentColor"
              class="w-6 h-6"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
              />
            </svg>
          </button>
        </div>
      </div>
    </div>
  </section>
</template>
